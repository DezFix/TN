...
Future<Chat?> showChatEditDialog(
  ...
  return showDialog<Chat>(
    ...
    builder: (ctx) => StatefulBuilder(
      builder: (ctx, setState) => AlertDialog(
        ...
        title: Text(
          tr(chat != null ? 'edit_chat' : 'new_chat'),
          style: TextStyle(
            fontSize: 16,
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Text(
                            ...
                            style: TextStyle(
                              fontSize: 12,
                              color: selectedKind == k.$1 ? p.accent : p.text,
                            ),
                          ),
                        ),
                      ),
...
Future<EntryAction?> showEntryCtxPopup(
  BuildContext context,
  AppModel model,
  Entry entry,
  Offset globalPos, {
  String chatKind = 'note',
  bool readOnly = false,
}) {
  final p = model.p;
  final tr = model.tr;
  final canEdit = !readOnly && (entry.type == 'text' || entry.type == 'todo');
  final isImage = entry.type == 'image';
  final showSchedule =
      !readOnly && (chatKind == 'tasks' || chatKind == 'kanban');
  ...
  return showMenu<EntryAction>(
    ...
    items: [
      if (showSchedule)
        PopupMenuItem(
          value: EntryAction.schedTime,
          child: Row(
            children: [
              Icon(Icons.schedule_outlined, size: 18, color: p.accent),
              const SizedBox(width: 10),
              Text(tr('change_time'), style: TextStyle(color: p.text)),
            ],
          ),
        ),
      if (!readOnly && chatKind == 'kanban')
        PopupMenuItem(
          value: EntryAction.boardMove,
          child: Row(
            children: [
              Icon(Icons.swap_horiz_rounded, size: 18, color: p.accent),
              const SizedBox(width: 10),
              Text(tr('board_move_to'), style: TextStyle(color: p.text)),
            ],
          ),
        ),
      if (!readOnly)
        PopupMenuItem(
          value: EntryAction.pin,
          child: Row(
            children: [
              Icon(
                Icons.push_pin_outlined,
                size: 18,
                color: entry.pinned ? p.accent : p.textSoft,
              ),
              const SizedBox(width: 10),
              Text(
                tr(entry.pinned ? 'unpin' : 'pin'),
                style: TextStyle(color: p.text),
              ),
            ],
          ),
      ...
      PopupMenuItem(
        value: EntryAction.select,
        child: Row(
          children: [
            Icon(Icons.checklist, size: 18, color: p.accent),
            const SizedBox(width: 10),
            Text(tr('select'), style: TextStyle(color: p.text)),
          ],
        ),
      ),
      if (canEdit)
        PopupMenuItem(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 10),
              Text(tr('copy'), style: TextStyle(color: p.text)),
            ],
          ),
        ),
      if (canEdit)
        PopupMenuItem(
          ...
          child: Row(
            children: [
              Icon(Icons.edit, size: 18, color: p.textSoft),
              const SizedBox(width: 10),
              Text(tr('edit'), style: TextStyle(color: p.text)),
            ],
          ),
        ),
      PopupMenuItem(
        value: EntryAction.forward,
        child: Row(
          children: [
            Icon(Icons.forward, size: 18, color: p.textSoft),
            const SizedBox(width: 10),
            Text(tr('forward'), style: TextStyle(color: p.text)),
          ],
        ),
      ),
      // external share
      PopupMenuItem(
        value: EntryAction.share,
        child: Row(
          children: [
            Icon(Icons.share, size: 18, color: p.accent),
            const SizedBox(width: 10),
            Text(
              tr(isImage ? 'share_photo' : 'share_text'),
              style: TextStyle(color: p.text),
            ),
          ],
        ),
      ...
      if (isImage)
        PopupMenuItem(
          ...
          child: Row(
            children: [
              Icon(Icons.download, size: 18, color: p.textSoft),
              const SizedBox(width: 10),
              Text(tr('download'), style: TextStyle(color: p.text)),
            ],
          ),
        ),
      if (!readOnly)
        PopupMenuItem(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 10),
              Text(tr('delete'), style: TextStyle(color: p.text)),
            ],
          ),
        ),
    ],
...
Future<bool> showBoardManageSheet(
  BuildContext context,
  AppModel model,
  Chat chat,
) {
...
class _BoardManageSheetState extends State<_BoardManageSheet> {
  ...
  Future<void> _add() async {
    ...
    final tr = model.tr;
    widget.chat.ensureBoard(tr);
    widget.chat.board!.add(
      BoardColumn(
        id: uid('b'),
        name: '${tr('board_add')} ${widget.chat.board!.length + 1}',
      ),
    );
    await model.save();
    if (mounted) setState(() {});
          ...
          children: [
            ...
            Flexible(
              child: ListView.separated(
                ...
                itemBuilder: (_, i) {
                  final b = boards[i];
                  final count = model.state.entries
                      .where(
                        (e) =>
                            e.chatId == widget.chat.id &&
                            (e.boardId ?? boards.first.id) == b.id,
                      )
                      .length;
                  return ListTile(
...
Future<EntryAction?> showEntryCtxSheet(
  BuildContext context,
  AppModel model,
  Entry entry,
) {
...
Future<Chat?> showForwardDialog(
  ...
  return showDialog<Chat>(
    ...
    builder: (ctx) => SimpleDialog(
      ...
      children: [
        for (final c in model.state.chats.where(
          (c) => !c.isTrashed && c.kind != 'rss',
        ))
          SimpleDialogOption(
        ...
        if (allowNewChat)
          SimpleDialogOption(
            onPressed: () async {
              ...
              final color =
                  appColors[model.state.chats.length % appColors.length];
              final chat = Chat(
                id: uid('c'),
                name: name.trim(),
                color: color,
    ...
    builder: (ctx) => StatefulBuilder(
      builder: (ctx, setState) {
        ...
        Future<void> pickTime() async {
          final initPrio =
              schedule?.priority ??
              (items.isNotEmpty ? items.first.priority : 0);
                ...
                Row(
                  children: [
                    Expanded(
                      child: Text(
                        tr(
                          entry != null ? 'todo_edit_title' : 'todo_new_title',
                        ),
                        style: TextStyle(
                          fontSize: 16,
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                schedule?.dueAt != null
                                    ? fmtDue(schedule!.dueAt!)
                                    : tr('sched_sheet_title'),
                                style: TextStyle(
                                  fontSize: 13,
                                  fontWeight: FontWeight.w700,
                                  color: schedule?.dueAt != null
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(
                          child: Text(
                            '${tr('todo_sub_hint')} • ${roots.firstWhere(
                              (r) => r.id == addUnder,
                              orElse: () => TodoItem(id: '', text: ''),
                            ).text.take(24)}',
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                            style: TextStyle(
                        ...
                        const SizedBox(width: 8),
                        GestureDetector(
                          onTap: () => setState(() => addUnder = null),
                          child: Icon(Icons.close, size: 14, color: p.accent),
                        ),
                      ],
                    ),
                  ),
                    ...
                    child: display.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            separatorBuilder: (_, __) => Divider(
                              height: 1,
                              color: p.divider.withValues(alpha: .35),
                              indent: 14,
                              endIndent: 14,
                            ),
                            itemBuilder: (ctx, i) {
                              ...
                              return Padding(
                                ...
                                child: Container(
                                  decoration: BoxDecoration(
                                    ...
                                    borderRadius: BorderRadius.circular(10),
                                    border: Border.all(
                                      color: p.divider.withValues(alpha: .45),
                                    ),
                                  ),
                                  child: Row(
                                    crossAxisAlignment:
                      ...
                      onPressed: () {
                        ...
                        if (cleaned.isEmpty) {
                          ScaffoldMessenger.of(ctx).showSnackBar(
                            SnackBar(content: Text(tr('todo_empty'))),
                          );
                          return;
                        }
...
Future<AttachOption?> showAttachMenuPopup(
  ...
  return showMenu<AttachOption>(
    ...
    items: [
      PopupMenuItem(
        value: AttachOption.photo,
        child: Row(
          children: [
            Icon(Icons.photo_outlined, size: 18, color: p.accent),
            const SizedBox(width: 10),
            Text(tr('attach_photo'), style: TextStyle(color: p.text)),
          ],
        ),
      ),
      PopupMenuItem(
        value: AttachOption.todo,
        child: Row(
          children: [
            Icon(Icons.checklist, size: 18, color: p.accent),
            const SizedBox(width: 10),
            Text(tr('attach_todo'), style: TextStyle(color: p.text)),
          ],
        ),
      ),
    ],
...
Future<ChatTopAction?> showChatTopMenuPopup(
  ...
  return showMenu<ChatTopAction>(
    ...
    items: [
      PopupMenuItem(
        value: ChatTopAction.remind,
        child: Row(
          children: [
            Icon(Icons.alarm, size: 18, color: p.accent),
            const SizedBox(width: 10),
            Text(tr('remind'), style: TextStyle(color: p.text)),
          ],
        ),
      ),
      PopupMenuItem(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 10),
            Text(tr('edit_chat'), style: TextStyle(color: p.text)),
          ],
        ),
      ),
      PopupMenuItem(
        ...
        child: Row(
          children: [
            Icon(Icons.delete_outline, size: 18, color: p.danger),
            const SizedBox(width: 10),
            Text(tr('delete'), style: TextStyle(color: p.danger)),
          ],
        ),
      ),
    ],
          ...
          children: [
            for (var d = 1; d <= 7; d++)
              FilterChip(
                ...
                onSelected: (v) =>
                    setState(() => v ? selected.add(d) : selected.remove(d)),
                labelStyle: TextStyle(
                  color: selected.contains(d) ? Colors.white : p.textSoft,
                ),
                      ...
                      onSelected: (_) async {
                        switch (v) {
                          ...
                          case 'weekdays':
                            ...
                            break;
                          case 'pickdays':
                            final d = await showWeekdayPickerDialog(
                              ctx,
                              model,
                              days,
                            );
                            ...
                            break;
                          case 'monthly':
                            final m = await showMonthDayPickerDialog(
                              ctx,
                              model,
                              initial: md,
                            );
...
Future<String?> showFolderNameDialog(
  ...
  return showDialog<String>(
    ...
    builder: (ctx) => AlertDialog(
      backgroundColor: p.modalBg,
      title: Text(
        tr(titleKey ?? (initial == null ? 'new_folder' : 'edit_folder')),
        style: TextStyle(color: p.text),
      ),
      content: TextField(
        controller: ctrl,
  ...
  return showDialog<Folder>(
    ...
    builder: (ctx) => StatefulBuilder(
      builder: (ctx, setState) => AlertDialog(
        backgroundColor: p.modalBg,
        title: Text(
          tr(folder == null ? 'new_folder' : 'edit_folder'),
          style: TextStyle(color: p.text),
        ),
...
Future<ChatAction?> showChatCtxPopup(
  BuildContext context,
  AppModel model,
  Chat chat,
  Offset globalPos,
  ...
  return showMenu<ChatAction>(
    ...
    items: [
      PopupMenuItem(
        value: chat.pinned ? ChatAction.unpin : ChatAction.pin,
        child: Row(
          children: [
      ...
      PopupMenuItem(
        ...
        child: Row(
          children: [
            Icon(Icons.folder_copy_outlined, size: 18, color: p.accent),
            const SizedBox(width: 10),
            Text(tr('move_to_folder'), style: TextStyle(color: p.text)),
          ],
        ),
      ),
      PopupMenuItem(
        value: ChatAction.edit,
        child: Row(
          children: [
            Icon(Icons.edit_outlined, size: 18, color: p.textSoft),
            const SizedBox(width: 10),
      ...
      PopupMenuItem(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 10),
            Text(tr('delete'), style: TextStyle(color: Colors.redAccent)),
          ],
        ),
      ),
    ],
...
Future<ChatAction?> showChatCtxSheet(
  BuildContext context,
  AppModel model,
  Chat chat,
) {
  final p = model.p;
  final tr = model.tr;
  Widget tile(IconData icon, String label, ChatAction action, {Color? color}) =>
      ListTile(
        leading: Icon(icon, color: color ?? p.accent),
        title: Text(
          label,
          style: TextStyle(fontSize: 15, color: color ?? p.text),
        ),
...
Future<String?> showMoveToFolderPopup(
  BuildContext context,
  AppModel model,
  Offset globalPos,
) {
  ...
  return showMenu<String>(
    ...
    items: [
      PopupMenuItem(
        ...
        child: Row(
          children: [
            Icon(Icons.folder_off_outlined, size: 18, color: p.accent),
            const SizedBox(width: 10),
            Text(tr('no_folder'), style: TextStyle(color: p.text)),
          ],
        ),
      ),
      for (final f in model.state.folders)
        PopupMenuItem(
          ...
          child: Row(
            children: [
              Icon(Icons.folder_outlined, size: 18, color: p.accent),
              const SizedBox(width: 10),
              Text(f.name, style: TextStyle(color: p.text)),
            ],
          ),
        ),
    ],
