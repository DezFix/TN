...
async def main():
    ...
    check(len(app.state.chats) == 3, 'init: 3 default chats')
    ...
    app.open_chat('c_saved')
    check(app.current_chat_id == 'c_saved', 'open chat')
    check(app.screen_chat.visible and not app.screen_list.visible, 'open chat: screens switched')
    ...
    check(len(app.state.chats) == 4, 'create chat: added')
    ...
    check(len(app.state.chats) == 3, 'delete chat: chat removed')
